...
<c:when test="${field == 'privateNote'}"><td class="${editable} ${collapsedClass}" data-name="privateNote">${taxon == null ? '' : taxon.getPrivateComment()}</td></c:when>
...
<%--<c:when test="${field == 'gpsCode'}"><td class="${editable} ${collapsedClass}" data-name="gpsCode">${taxon == null ? '' : inventory.getCode()}</td></c:when>--%>
