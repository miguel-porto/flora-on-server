...
<%-- <c:when test="${field == 'coordinates' && !noInventory}"><c:set var="coordchanged" value="${taxon == null ? '' : (taxon.getCoordinatesChanged() ? 'textemphasis' : '')}" /><td class="${editable} ${collapsedClass} ${coordchanged} hideincompactview" data-name="observationCoordinates">${taxon == null ? '' : inventory._getCoordinates()}</td></c:when> --%>
