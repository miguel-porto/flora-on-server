...
<c:if test="${width == null}"><div class="ajaxcontent ${classes}" data-url="${url}" id="${id}"><img src="${contextPath}/images/loader.svg" class="ajaxloader"/></div></c:if>
...
<div class="ajaxcontent ${classes}" style="position:relative; width:${width}; height:${height};display:inline-block" data-url="${url}" <c:if test="${id != null}">id="${id}"</c:if> >
    <img style="position:absolute; width:100%; height:100%; left:0; top:0" src="${contextPath}/images/loader.svg" class="ajaxloader"/>
    <p>${text}</p>
</div>
</c:if>