...
<div id="redlistcategories">
    <c:if test="${user.canEDIT_9_1_2_3_4() || user.canEDIT_SECTION9()}">
        <c:forEach var="tmp" items="${assessment_Category}">
            <c:if test="${rlde.getAssessment().getFinalCategory().getEffectiveCategory().equals(tmp)}">
                <input type="radio" name="assessment_Category" value="${rlde.getAssessment().getFinalCategory().toString()}" id="assess_${tmp.toString()}" checked="checked" class="trigger" data-trigger="${tmp.isTrigger() ? 1 : 0}">
            </c:if>
            <c:if test="${!rlde.getAssessment().getFinalCategory().getEffectiveCategory().equals(tmp)}">
            ...
            <label for="assess_${tmp.toString()}">
                <h1>
                    ${tmp.getShortTag()}<c:if test="${rlde.getAssessment().getFinalCategory().getEffectiveCategory().equals(tmp) && rlde.getAssessment().getFinalCategory().isUpDownListed()}">º</c:if>
