...
/**
 * A parser which looks for partial matches anywhere in each word or word sequence.
 * It runs iteratively, starting with multiple-word sequences.
 * @author miguel
 *
 */
public class TaxonPartialParser extends TokenParser {
	private Integer iteration=0;
	private String[] collectionRestrictions=new String[] {NodeTypes.taxent.toString()};
	
	public TaxonPartialParser(FloraOnGraph graph, QueryString query) {
		super(graph, query);
	}
...
/*
		...
		TaxonPartialParser ptpp=new TaxonPartialParser(this.graph,this.curquery);
