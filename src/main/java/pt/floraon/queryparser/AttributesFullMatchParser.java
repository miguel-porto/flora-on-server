...
public class AttributesFullMatchParser extends TokenParser {
	private Integer iteration=0;
	private String[] collectionRestrictions=new String[] {NodeTypes.attribute.toString()};

	public AttributesFullMatchParser(FloraOnGraph graph, QueryString query) {
		super(graph, query);
	}
	...
	public QueryString parse() {
		...
		for(int whichPiece=0;whichPiece<this.curquery.query.length;whichPiece++) {
			...
			groupWordsIn=splitpieces.length-this.iteration;
...
/*
		...
		AttributesFullMatchParser ptpp=new AttributesFullMatchParser(this.graph,this.curquery);
